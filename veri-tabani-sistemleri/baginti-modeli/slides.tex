...
\begin{frame}
  ...
  \begin{ornek}[\texttt{MOVIE} bağıntısının \texttt{DIRECTORID} niteliği]
    \begin{columns}[t]
      \column{.6\textwidth}
      \begin{tiny}
      \begin{table}
...
2259 & Dario Argento \\\hline
 ...
 ... & ...           \\\hline
  ...
  \begin{picture}(100,50)(-140,-90)
    ...
    \only<3->{
      \put(0,41){\oval(45,9)}          % movie.directorid
      \put(90,41){\oval(20,9)}         % person.id
      \put(24,41){\vector(1,0){56}}    % movie.directorid->person.id
    }
  \end{picture}
\end{frame}
...
\underline{MOVIEID} & \underline{ACTORID} & ORD\\[2pt]\hline\hline
  ...
  \begin{picture}(210,80)(-25,-126)
    ...
    \thicklines
    \only<2->{
      \put(105,78){\oval(45,9)}        % movie.directorid
      \put(209,78){\oval(20,9)}        % person.id
      \put(128,78){\vector(1,0){71}}   % movie.directorid->person.id
    }
    \only<3->{
      \put(96,4){\oval(35,9)}          % casting.movieid
      \put(0,78){\oval(20,9)}          % movie.id
      \put(96,9){\vector(-4,3){88}}    % casting.movieid->movie.id
    }
    \only<4->{
      \put(136,4){\oval(35,9)}         % casting.personid
      \put(150,9){\vector(3,4){49}}    % casting.personid->person.id
    }
  \end{picture}
\end{frame}
...
CONSTRAINT constraint_name
  ...
  \begin{itemize}
    \item \texttt{<=} işlemi $\subseteq$ diye okunabilir
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[\texttt{PERSON} ve \texttt{CASTING} bağıntılarının yaratılması]
    \begin{lstlisting}
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[dış anahtarların tanımlanması]
    \begin{lstlisting}
CONSTRAINT MOVIE_FKEY_DIRECTOR
  MOVIE { DIRECTOR# }
      RENAME (DIRECTOR# AS PERSON#)
