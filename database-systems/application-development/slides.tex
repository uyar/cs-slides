...
\begin{frame}[fragile]
  ...
  \begin{example}[adding - using statement]
    \begin{lstlisting}[language=Java]
...
\begin{frame}[fragile]
  ...
  \begin{example}[adding - using prepared statement]
    \begin{lstlisting}[language=Java]
String query = "INSERT INTO MOVIE (TITLE, YR)"
    + " VALUES (?, ?)";
...
\begin{frame}[fragile]
  ...
  \begin{example}[reusing a prepared statement]
    \begin{lstlisting}[language=Java]
...
\begin{frame}[fragile]
  ...
  \begin{example}[reusing a prepared statement - cont'd]
    \begin{lstlisting}[language=Java]
for (int year : years) {
  stmt.setInt(1, year);
  ResultSet results = stmt.executeQuery();
