...
\begin{frame}
  ...
  \begin{example}[the \texttt{DIRECTORID} attribute of the \texttt{MOVIE} relation]
    \begin{columns}[t]
      \column{.6\textwidth}
      \begin{tiny}
      \begin{table}
...
\begin{frame}[fragile]
  ...
  \begin{example}[creating the \texttt{PERSON} and \texttt{CASTING} relations]
    \begin{lstlisting}
...
\begin{frame}[fragile]
  ...
  \begin{example}[defining the foreign keys]
    \begin{lstlisting}
CONSTRAINT MOVIE_FKEY_DIRECTOR
  MOVIE { DIRECTOR# }
      RENAME (DIRECTOR# AS PERSON#)
