...
\begin{frame}
  ...
  \begin{columns}[t]
    \column{.55\textwidth}
    \begin{itemize}
      \item set notation: $\{a_1,a_2,\dots,a_n\}$
      \item let $S$ be a set, and\\
        $a$ be an element
      \begin{itemize}
...
\begin{frame}
  ...
  \begin{proof}
    \begin{itemize}
      ...
      \item $z$ is not divisible by a prime number in the range $[2, s]$
      \pause
      \begin{enumerate}
        \item either $z$ is a prime number (note that $z > s$): $\neg Q$

        \pause
        \item or $z$ is divisible by a prime number $t$ where $t > s$: $\neg Q$
      \end{enumerate}
    \end{itemize}
  \end{proof}
\end{frame}
