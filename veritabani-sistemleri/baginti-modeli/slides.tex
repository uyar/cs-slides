  ...
  \begin{ornek}[film baðýntýsýnýn yüklemi]
    \begin{itemize}
      \item \texttt{TITLE} baþlýklý film \texttt{YR} yýlýnda \texttt{DIRECTOR}
        tarafýndan \texttt{COUNTRY} ülkesinde çekilmiþtir; kullanýlan
        \texttt{VOTES} oyun ortalamasý \texttt{SCORE}'dur.
...
\begin{frame}
  \frametitle{Deðer Kümesi}

  \begin{itemize}
    \item ayný niteliðe iliþkin bütün deðerler ayný deðer kümesinden seçilmeli

    \begin{itemize}
      \item karþýlaþtýrma iþlemi ancak ayný deðer kümesinden seçilmiþ deðerler
        arasýnda anlamlý
    \end{itemize}
...
\begin{frame}
  \frametitle{Deðer Kümesi Örneði}
  ...
  \begin{ornek}
    \begin{itemize}
      ...
      \begin{itemize}
        ...
        \item \texttt{YR} ve \texttt{VOTES} deðerleri birer tamsayý ancak
          bunlarý karþýlaþtýrmak anlamlý deðil
      \end{itemize}
    \end{itemize}
  \end{ornek}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[t]
    \column{.5\textwidth}
    \begin{itemize}
      \item çoklu için o niteliðin deðeri bilinmiyor
    \end{itemize}
    ...
    \column{.5\textwidth}
    \begin{itemize}
      \item çoklu o nitelik için bir deðer taþýmýyor
    \end{itemize}

    \begin{ornek}
      \begin{itemize}
        \item \texttt{Star Wars} filmi için henüz kimse oy kullanmamýþ,\\
...
\begin{frame}
  ...
  \begin{itemize}
    \item bilinmeyen nitelik deðerleri için boþ deðer yerine varsayýlan bir
      deðer kullanýlabilir

    \begin{itemize}
      \item varsayýlan deðer, niteliðin geçerli deðerleri arasýnda yer almamalý
    \end{itemize}
  \end{itemize}
  ...
  \begin{ornek}
    \begin{itemize}
      \item \texttt{SCORE} niteliði için varsayýlan deðer olarak \texttt{-1}
        seçilebilir
    \end{itemize}
  \end{ornek}
\end{frame}
...
INSERT INTO tablo_adý
  ...
  \begin{itemize}
    ...
    \item sütun adlarý belirtilmezse sütun deðerleri tablonun yaratýlmasý
      sýrasýnda belirtilen sýrayla verilmelidir
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{ornek}
    \begin{itemize}
      \item \texttt{YR} deðerleri \texttt{1880}'den küçük olamaz
      \item \texttt{SCORE} deðerleri \texttt{0} ile \texttt{10} arasýnda
        olmalýdýr
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[YR deðerleri 1880'den küçük olamaz]
    \begin{lstlisting}
CREATE TABLE MOVIE (
  TITLE VARCHAR(80),
  YR NUMERIC(4),
...
\begin{frame}[fragile]
  \frametitle{Deðer Kümesi Yaratma}

  \begin{block}{Komut Yazýmý}
    \begin{lstlisting}
CREATE DOMAIN deðer_kümesi_adý [ AS ] taban_tip
  [ DEFAULT varsayýlan_deðer ]
  [ kýsýtlama_tanýmlarý ]
    \end{lstlisting}
  \end{block}
  ...
  \pause
  \begin{itemize}
    \item yaratýlan deðer kümesi sütun tipi olarak kullanýlabilir
  \end{itemize}

  \pause
  \begin{block}{Deðer Kümesi Silme}
    \begin{lstlisting}
DROP DOMAIN deðer_kümesi_adý [, ...]
    \end{lstlisting}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[yýllarýn tanýmlandýðý deðer kümesi]
    \begin{lstlisting}
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[SCORE niteliði 0'dan küçük olamasýn]
    \begin{lstlisting}
ALTER TABLE MOVIE
  ...
  \pause
  \begin{ornek}[SCORE niteliði 0'dan küçük olabilsin]
    \begin{lstlisting}
ALTER TABLE MOVIE
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item $A$'nýn bir anahtar adayý olabilmesi için þu koþullar saðlanmalýdýr:

    \pause
    \begin{itemize}
      \item \alert{eþsizlik}: herhangi iki çoklu $A$'da yer alan bütün
        nitelikler için ayný deðeri taþýyamazlar

      \pause
      \item \alert{indirgenemezlik}: $A$'nýn hiçbir altkümesi eþsizlik
        özelliðini saðlamaz
    \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \pause
    \item birincil anahtarýn parçasý olan hiçbir niteliðin deðeri hiçbir
      çokluda boþ olamaz

    \pause
    \item birincil anahtarý oluþturan nitelikler altýçizili gösterilir
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{ornek}[film veritabanýndaki dýþ anahtarlar]
    \begin{columns}
      \column{.6\textwidth}
      \begin{tiny}
      \begin{table}
...
\begin{frame}
  ...
  \begin{tanim}
    \alert{baþvuru bütünlüðü}:\\
      dýþ anahtar niteliðinin aldýðý bütün deðerler baþvurulan baðýntýnýn
      ilgili niteliðinde bulunmalý
  \end{tanim}

  \pause
  \begin{itemize}
    \item yapýlmasý istenen iþlem baþvuru bütünlüðünü bozacaksa ne olacak?
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[silerken kýsýtla]
    \begin{itemize}
      \item \texttt{PERSON} baðýntýsýndan silinmek istenen yönetmenin
        \texttt{ID} niteliði deðeri \texttt{MOVIE} baðýntýsýnýn
        \texttt{DIRECTORID} niteliði deðerleri arasýnda geçiyorsa silmeye izin
        verme
    \end{itemize}
  \end{ornek}
  ...
  \begin{ornek}[güncellerken yansýt]
    \begin{itemize}
      \item bir yönetmenin \texttt{ID} niteliði deðeri \texttt{PERSON}
        baðýntýsýnda güncellendiðinde \texttt{MOVIE} baðýntýsýnýn
        \texttt{DIRECTORID} niteliði deðerlerinde geçtiði çoklularda da güncelle
    \end{itemize}
  \end{ornek}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[silerken yansýt]
    \begin{itemize}
      ...
      \pause
      \item yönetmenin \texttt{ID} niteliði deðerinin \texttt{MOVIE}
        baðýntýsýnýn \texttt{DIRECTORID} niteliðinde geçtiði bütün çoklularý
        \texttt{MOVIE} baðýntýsýndan sil

      \pause
      \item silinen her bir \texttt{MOVIE} baðýntýsý çoklusunun \texttt{ID}
        niteliði deðerinin \texttt{CASTING} baðýntýsýnýn \texttt{MOVIEID}
        niteliðinde geçtiði bütün çoklularý \texttt{CASTING} baðýntýsýndan sil
    \end{itemize}
  \end{ornek}
\end{frame}
