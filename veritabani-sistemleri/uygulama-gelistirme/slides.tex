...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item gerektikçe:
    \begin{itemize}
      \item sorgu çalýþtýrýp sonucu al
      \item sorgu sonucu üzerinde döngüyle satýr satýr gez
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Ýmleçler}
    \begin{lstlisting}[language=EmbeddedSQL]
EXEC SQL DECLARE imleç CURSOR FOR SELECT...
...
\begin{frame}
  \frametitle{JDBC Paketi}

  \begin{itemize}
    \item kullanmak için JDBC paketi projeye eklenmeli:\\
      \lstinline!import java.sql.*!

    \pause
    \medskip
    \item SQL ile ilgili hatalar için aykýrý durum:\\
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item parametreler için yer tutucu: '\lstinline!?!'
    ...
    \begin{itemize}
      ...
      \item veriyi uygun tipte okuma metodlarý
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[veritabaný sürücüsünün yoklanmasý]
...
\begin{frame}[fragile]
  ...
  \begin{ornek}[önceden derlenmiþ komut]
    \begin{lstlisting}[language=Java]
...
\begin{frame}[fragile]
  ...
  \begin{block}{Yetki Verme}
    \begin{lstlisting}[language=ExtendedSQL]
GRANT yetki_adý ON nesne_adý TO özne_adý
  [ WITH GRANT OPTION ]
    \end{lstlisting}
  \end{block}

  \pause
  \begin{block}{Yetki Kaldýrma}
    \begin{lstlisting}[language=ExtendedSQL]
REVOKE yetki_adý ON nesne_adý FROM özne_adý
    \end{lstlisting}
  \end{block}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Yetki Örnekleri}

  \begin{ornek}[tablo için yetki verme]
    \begin{lstlisting}[language=ExtendedSQL]
GRANT SELECT, INSERT, UPDATE ON MOVIE TO 'itucs'
  ...
  \pause
  \begin{ornek}[görüntü için yetki verme]
    \begin{lstlisting}[language=ExtendedSQL]
GRANT SELECT ON NEW_MOVIE TO 'itucs'
  ...
  \pause
  \begin{ornek}[tablo için yetki kaldirma]
    \begin{lstlisting}[language=ExtendedSQL]
